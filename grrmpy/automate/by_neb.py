...
#                  qtraj = "Q_Str.traj"):
#         self.calc = calc_func
#         self.p0 = reactant.copy()
#         self.pi = reactant.copy()
#         self.p0.calc = pfp_calculator()
#         self.pi.calc = pfp_calculator()
#         self.q0 = product.copy()
#         self.qi = product.copy()
#         self.q0.calc = pfp_calculator()
#         self.qi.calc = pfp_calculator()
#         self.optimizer = optimier
#         self.logfile = logfile
#         self.maxstep = maxstep
#         self.ptraj = ptraj
#         self.qtraj = qtraj
#         Trajectory(filename=self.ptraj,mode="a",atoms=self.p0).write()
#         Trajectory(filename=self.qtraj,mode="a",atoms=self.q0).write()
        
#     def run(self,dif=0.12,fmax=0.0001,steps=1):
#         """

#         Parameters:
        
#         dif: float
#             |qi-pj| < difになるまで計算を行なう. 収束条件
#         fmax:
#             difを収束条件としたいので低い値に設定している. この値をいじることはまずない.
#         steps:
#             基本は1
#         """
#         while not np.linalg.norm(self.qi-self.pj) < dif:
#             if self.qi.get_potentail_energy()-self.pj.get_potentail_energy():
#                 # self.qi.set_constraint(ForDsAFIR())#######何か
#                 self.opt = self.optimizer(self.qi,logfile=self.logfile,maxstep=self.maxstep)
...
# class ForDsAFIR(FixConstraint):
#     def __init__(self,ds_afir:DS_AFIR):
#         self.ds_afir = ds_afir

#     def calc_X(self,qi,pj,delta):
#         """
#         X = delta/|u| - gi・u/|u|^2
#         """
#         u = self.calc_u(qi,pj)
#         term0 = delta/np.linalg.norm(u,axis=1).reshape(-1, 1)
#         g1 = qi.get_forces()
#         tarm1 = (g1*u).sum(1).reshape(-1, 1)/np.linalg.norm(u)**2
#         return term0 - tarm1
    
#     def calc_Y(self,qi,pj):
#         """
#         Y = Z/(1+Z)  if Z>0 else Y=0
#         列ベクトルを返す,1つ1つの要素が1つの原子に対応する
#         """
#         z = self.calc_Z(qi,pj)
#         y = np.where(z > 0, z/(1+z), 0)
#         return y
    
#     def calc_Z(self,qi,pj): # p0(ini), q0(fin)
#         """
#         Z = |qi-q0|/|qi-pj| + (qi-pj)・(qi-p0)/|qi-pj||qi-q0|
#         列ベクトルを返す,1つ1つの要素が1つの原子に対応する
#         """
#         qi_q0 = qi.positions - self.ds_afir.q0.positions
#         abs_qi_q0 = np.linalg.norm(qi_q0,axis=1).reshape(-1, 1)
#         qi_pj = qi.positions - pj.positions
#         abs_qi_pj = np.linalg.norm(qi_pj,axis=1).reshape(-1, 1)
#         term0 = abs_qi_q0 / abs_qi_pj

#         qi_p0 = qi.positions - self.ds_afir.p0.positions
#         term1 = (qi_pj*qi_p0).sum(1).reshape(-1, 1) / (abs_qi_pj*abs_qi_q0)
#         return term0 + term1
    
#     def calc_u(self,qi,pj):
#         """
#         u = Y (qi-pj)/|qi-pj| - (1-Y) (qi-q0)/|qi-qj|
#         3*nの行列を返す
#         """
#         y = self.calc_Y(qi,pj)
#         qi_pj = qi.positions - pj.positions
#         abs_qi_pj = np.linalg.norm(qi_pj,axis=1).reshape(-1, 1)
#         qi_p0 = qi.positions - self.ds_afir.p0.positions
#         term0 = y*(qi_pj/abs_qi_pj)
#         term1 = (1-y)*(qi_p0/abs_qi_pj)
#         return term0 - term1
    
#     def adjust_forces(self, atoms, forces):
#         """
#         F_dsafir(qi) = E(qi) + XY|qi-pj| -X(1-Y)|qi-q0|
#         E(qi)は計算しない
#         """
#         x = self.calc_X(qi,pj,delta)
#         y = slef.calc_Y(qi,pj)
#         qi_pj = qi.positions-pj.positions
#         qi_q0 = qi.positions-pj.positions
#         return f_afir
#         forces += f_afir