...
from grrmpy.structure.structure import EQ, TS, PT
...
def view(atoms):
    ...
    elif type(atoms) == EQList or type(atoms) == TSList or type(atoms) == PTList:
        atoms = atoms.get_atoms_list()
        _view_atomslist(atoms)
    elif type(atoms)==Atoms:
        """Atomsで与えられた場合"""
        _view_atoms(atoms)
    elif type(atoms) == EQ or type(atoms) == TS or type(atoms) == PT:
...
def view_images(imagess):
    ...
    elif type(imagess) == EQList or type(imagess) == TSList or type(imagess) == PTList:
        imagess = imagess.get_atoms_list()
        _viewimage_1datomslist(imagess)
    else:
        raise TypeError("引数が間違っています")
